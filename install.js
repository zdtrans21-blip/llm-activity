...
const ACTIVITY_PARAMS = {
  ...
  RETURN_PROPERTIES: {
    llm_result: {
      Name: 'Результат LLM',
      Description: 'Полный текст ответа LLM. Если задание предполагает разбивку на пункты, каждый пункт помечен эмодзи-статусом.',
      Type: 'string'
    },
    llm_status: {
    ...
    llm_issues: {
      ...
      Description: 'Только пункты с замечаниями или ошибками, без пунктов в порядке. Если замечаний нет — соответствующее сообщение. Заполняется, только если задание предполагает разбивку на отдельные пункты.',
      Type: 'string'
    }
  }
};
